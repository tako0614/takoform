...
func (c *Client) PrepareResource(ctx context.Context, resource *Resource, fence Fence) (*PrepareResult, error) {
	...
	if err := validatePrepareResponseWire(data); err != nil {
		return nil, fmt.Errorf("takoform: prepare response is protocol-invalid: %w", err)
	}
...
func (c *Client) finishResourceMutation(
	...
	if status == http.StatusAccepted {
		...
		if err := validateResourceResponseWire(terminal.Result, true); err != nil {
			return nil, acceptedMutation(operationID, uid,
				fmt.Errorf("takoform: terminal resource response is protocol-invalid: %w", err))
		}
		var envelope struct {
	...
	if err := validateResourceResponseWire(data, false); err != nil {
		return nil, acceptedMutation("", "",
			fmt.Errorf("takoform: resource response is protocol-invalid: %w", err))
	}
	var out Resource
...
func (c *Client) GetResource(ctx context.Context, space string, ref FormRef, name string) (*Resource, error) {
	...
	if err := validateResourceResponseWire(data, false); err != nil {
		return nil, fmt.Errorf("takoform: resource response is protocol-invalid: %w", err)
	}
	var out Resource
	if err := decodeBody(data, fullURL, &out); err != nil {
		return nil, err
...
func (c *Client) fencedStatusAction(ctx context.Context, action, space string, ref FormRef, name, uid, generation string) (*Resource, error) {
	...
	if err := validateResourceResponseWire(data, true); err != nil {
		return nil, fmt.Errorf("takoform: resource response is protocol-invalid: %w", err)
	}
	var envelope struct {
		Resource Resource `json:"resource"`
...
// ImportResource adopts one existing host-native object using the full
