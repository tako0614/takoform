...
export const HAND_AUTHORED_PAGE_SOURCES = Object.freeze([
  ...JAPANESE_PAGE_SOURCES,
  ...JAPANESE_PAGE_SOURCES.map((source) => source.replace(`${SITE_ROOT}/`, `${SITE_ROOT}/en/`)),
]);
...
function schemaIndexPage(schemas, locale = "ja") {
  const rows = (entries) =>
    entries.map((entry) =>
  ...
  const title = locale === "en" ? "Public schemas" : "公開 schema";
...
${locale === "en" ? `This non-normative index is derived from the append-only identity ledger. It
...
current-authoring guidance before selecting an identity.` : `公開スキーマの記録から生成しています。この索引は案内用であり、仕様ではありません。
新しい識別子が記録されても、固定されたHost API v1の仕様に追加されたり、既存の契約が変わったりするわけではありません。

activeの一覧には、新規作成に使うもの（current-authoring）と、既存データを読むためのもの（retained-readable）が混在します。
activeは廃止されていないことだけを示し、新規作成に適していることまでは示しません。選ぶ前に各仕様の用途を確認してください。`}

${locale === "en" ? `Normative schemas are served at the paths named by their \`$id\` values.
The served bytes are identical to the source in
[\`spec/schemas/\`](${SOURCE_TREE}/spec/schemas); their digests are recorded in
the append-only [identity ledger](${SOURCE_BROWSE}/release/public-schema-identities.json).

An \`$id\` is a logical specification identity. The publishing operator decides
which hostnames route to this site; this repository does not choose them.

Schemas validate structural requirements, not the whole contract. Semantic
constraints are defined by [conformance](/en/spec/conformance) and each contract.` : `スキーマは\`$id\`が示すパスで、原文のまま配信します。
配信内容は[\`spec/schemas/\`](${SOURCE_TREE}/spec/schemas)とバイト単位で一致し、ダイジェストは追記専用の
[\`release/public-schema-identities.json\`](${SOURCE_BROWSE}/release/public-schema-identities.json)
に記録されています。

\`$id\`は仕様上の識別子です。実際にどのホスト名をサイトに接続するかは、公開する運用者が決めます。

スキーマによる構造の検証だけでは、仕様を満たすとは限りません。意味上の制約は
[適合性の仕様](/spec/conformance)と各仕様本文を確認してください。`}

## Active identity（${active.length}）

${locale === "en" ? `The active section is the non-retired half of that mixed inventory. Its entries
may be current-authoring or retained-readable; the source contract decides
which role applies.

For the exact role assigned to each identity, use the [schema role table](${prefix}/spec/schemas/)
before selecting an entry for authoring or retained-readable verification.` : `廃止されていない識別子の一覧です。新規作成に使うものか、既存データの読み取り用かは仕様ごとに異なります。
選択する前に[スキーマの用途一覧](/spec/schemas/)を確認してください。`}

| ${locale === "en" ? "Served path" : "配信 path"} | \`$id\` | digest | source |
| --- | --- | --- | --- |
${rows(active)}

## Verify-only identity（${verifyOnly.length}）

${locale === "en" ? "These identities have been retired but remain occupied. They are retained only for reading existing documents, not for issuing new documents. Their original bytes do not change." : "廃止された識別子です。識別子は再利用せず、既存データを読むためだけに残します。新しいデータの発行には使えません。配信内容は発行時のまま変わりません。"}
...
export function buildSiteFiles(root) {
  ...
  files.set(`${SITE_ROOT}/en/schemas/index.md`, Buffer.from(schemaIndexPage(schemas, "en"), "utf8"));
  return files;
}
...
export function inspectDist(root, distRoot = SITE_DIST) {
  ...
  for (const source of [...HAND_AUTHORED_PAGE_SOURCES, ...GENERATED_INDEX_PAGES]) {
    expectedPageMetadata.set(distPagePathForSource(source, distRoot), {
      route: siteRouteForPageSource(source),
      lang: source.startsWith(`${SITE_ROOT}/en/`) ? "en" : "ja-JP",
      authority: "non-normative",
      mirror: false,
    });
  }
