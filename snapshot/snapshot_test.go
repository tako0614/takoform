...
func TestHostAPILaneComparisonRetainsHistoricalLowerBounds(t *testing.T) {
	for _, test := range []struct {
		...
		{name: "unknown lane fails closed", served: "forms.takoform.com/v9", required: "forms.takoform.com/v1", want: false},
	} {
...
func TestCompileRefusesUnknownOrInsufficientHostAPILanes(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{name: "unknown syntactically valid lane", hostAPI: "forms.takoform.com/v9beta1", code: DiagnosticInvalidInput},
...
func TestCompileRetainsVerifiedHistoricalPackageProfile(t *testing.T) {
	definition := map[string]any{
		"apiVersion":        "resources.publisher.example/v1beta1",
		"kind":              "HistoricalStore",
	...
	artifact, ref := packageFromDefinitionGeneration(t, definition, formpackage.FamilyPackageAPIVersion)
	if _, err := formpackage.ValidateDefinition(artifact.Package.Definition()); err == nil {
		t.Fatal("historical Definition unexpectedly passed the direct current-profile API")
	}
	compiled, diagnostics := Compile(Input{
		HostAPI:        "forms.takoform.com/v1",
		Packages:       []PackageArtifact{artifact},
		DefaultCreates: []DefaultPin{{Group: ref.APIVersion, Kind: ref.Kind, Ref: ref}},
	})
	if compiled == nil || len(diagnostics) != 0 {
		t.Fatalf("retained package failed Snapshot compilation: compiled=%v diagnostics=%#v", compiled != nil, diagnostics)
	}
}
...
func packageFromDefinition(t *testing.T, definition map[string]any) (PackageArtifact, formpackage.FormRef) {
	t.Helper()
	return packageFromDefinitionGeneration(t, definition, formpackage.VersionlessFamilyPackageAPIVersion)
}

func packageFromDefinitionGeneration(t *testing.T, definition map[string]any, packageAPIVersion string) (PackageArtifact, formpackage.FormRef) {
	t.Helper()
