...
1. 公開元が管理する名前空間と、Formの用途・振る舞いを決めます。
2. 設定、更新・削除、エラー、再試行の規則と、それを確認するテストを用意します。
3. [互換性の規則](/spec/versioning) に従ってバージョンを決め、公開済みの内容を上書きしません。
4. [署名と失効の仕様](/spec/trust/) に沿った来歴・署名・失効情報と、利用例・制約の説明を公開元で用意します。
5. 利用するHostが、その正確なFormRefを実装し、対象の利用者に許可しているかを確認します。
